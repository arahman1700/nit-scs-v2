...
describe('generateDocumentPdf', () => {
  ...
  it('creates a PDF and saves it with the docNumber as filename', () => {
    generateDocumentPdf(baseOptions);
    expect(mockSave).toHaveBeenCalledWith('GRN-2026-0001.pdf');
  });

  it('renders the NIT branding header', () => {
    generateDocumentPdf(baseOptions);
  ...
  it('renders the document number on the page', () => {
    generateDocumentPdf(baseOptions);
    expect(mockText).toHaveBeenCalledWith('GRN-2026-0001', 14, 52);
  });

  it('renders status and date right-aligned', () => {
    generateDocumentPdf(baseOptions);
  ...
  it('calls autoTable when lineItems are provided', () => {
    generateDocumentPdf({
      ...baseOptions,
  ...
  it('does not call autoTable when no lineItems provided', () => {
    generateDocumentPdf(baseOptions);
    expect(autoTable).not.toHaveBeenCalled();
  });

  it('opens print dialog in printMode instead of saving', () => {
    generateDocumentPdf({ ...baseOptions, printMode: true });
  ...
  it('handles odd number of fields (last field without pair)', () => {
    generateDocumentPdf({
      ...baseOptions,
      fields: [
        { label: 'Supplier', value: 'ACME Corp' },
...
describe('generateReportPdf', () => {
  ...
  it('creates a PDF and saves with title-based filename', () => {
    generateReportPdf(baseReportOptions);
    expect(mockSave).toHaveBeenCalledWith('Inventory_Report_Report.pdf');
  });

  it('replaces spaces with underscores in filename', () => {
    generateReportPdf({ ...baseReportOptions, title: 'My Custom  Report' });
  ...
  it('renders subtitle when provided', () => {
    generateReportPdf({ ...baseReportOptions, subtitle: 'Q1 2026' });
  ...
  it('renders summary cards when provided', () => {
    generateReportPdf({
      ...baseReportOptions,
  ...
  it('calls autoTable for the data table', () => {
    generateReportPdf(baseReportOptions);
    expect(autoTable).toHaveBeenCalled();
  });
});
...
describe('getStartY', () => {
  it('returns the stored __nitStartY value', () => {
    const doc = createNitPdf({ title: 'Test' });
...
describe('addInfoSection', () => {
  it('returns a Y position greater than the startY', () => {
    const doc = createNitPdf({ title: 'Test' });
    vi.clearAllMocks();
  ...
  it('increments Y for each field', () => {
    const doc = createNitPdf({ title: 'Test' });
    vi.clearAllMocks();
    const fields = [
      { label: 'A', value: '1' },
...
describe('addTable', () => {
  it('calls autoTable with the provided columns and data', () => {
    const doc = createNitPdf({ title: 'Test' });
    vi.clearAllMocks();
    const columns = [
    ...
    addTable(doc, columns, data, 50);
    expect(autoTable).toHaveBeenCalled();
  });

  it('returns a numeric Y position', () => {
    const doc = createNitPdf({ title: 'Test' });
    const y = addTable(doc, [{ header: 'H', dataKey: 'h' }], [{ h: 'val' }], 50);
    expect(typeof y).toBe('number');
  });

  it('uses getStartY when no startY is provided', () => {
    const doc = createNitPdf({ title: 'Test' });
    vi.clearAllMocks();
    addTable(doc, [{ header: 'H', dataKey: 'h' }], [{ h: 'val' }]);
...
describe('downloadPdf', () => {
  it('calls doc.save with .pdf extension', () => {
    const doc = createNitPdf({ title: 'Test' });
    vi.clearAllMocks();
  ...
  it('adds footers to all pages before saving', () => {
    const doc = createNitPdf({ title: 'Test' });
...
describe('addWatermark', () => {
  it('draws default "DRAFT" watermark text', () => {
    const doc = createNitPdf({ title: 'Test' });
    vi.clearAllMocks();
    addWatermark(doc);
  ...
  it('draws custom watermark text', () => {
    const doc = createNitPdf({ title: 'Test' });
    vi.clearAllMocks();
...
describe('addPageBreakIfNeeded', () => {
  ...
  it('does not add a page when sufficient space exists', () => {
    const doc = createNitPdf({ title: 'Test' });
