...
// Lazy-load jsPDF + autoTable — defers ~400KB until first PDF generation
type AutoTableFn = typeof import('jspdf-autotable').default;
type JsPDFCtor = typeof import('jspdf').default;

let _cached: { JsPDF: JsPDFCtor; autoTable: AutoTableFn } | null = null;

async function loadPdfModules() {
  if (!_cached) {
    const [jspdfMod, autoTableMod] = await Promise.all([import('jspdf'), import('jspdf-autotable')]);
    _cached = { JsPDF: jspdfMod.default, autoTable: autoTableMod.default };
  }
  return _cached;
}
...
export async function generateDocumentPdf(options: DocumentPdfOptions) {
  ...
  if (options.printMode) {
    doc.autoPrint();
    (globalThis as unknown as Window).open(doc.output('bloburl'), '_blank');
  } else {
...
// ---------------------------------------------------------------------------
// Section 2: NIT letterhead-style PDF generators
// ---------------------------------------------------------------------------

// NIT brand colors
export const PRIMARY_BLUE = '#1e3a5f';
 ...
 */
export async function createNitPdf(options: PdfOptions): Promise<jsPDF> {
...
export async function addTable(
  ...
  const { autoTable } = await loadPdfModules();
  autoTable(doc, {
    startY: y,
...
// ---------------------------------------------------------------------------
// Section 8: Report PDF generator
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Section 9: buildPdfOptions — Generic resource to PDF mapper
// ---------------------------------------------------------------------------
