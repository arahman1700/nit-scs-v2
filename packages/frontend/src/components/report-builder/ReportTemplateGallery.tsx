...
import { BarChart3, Table, PieChart, LineChart, Copy, Loader2, Share2 } from 'lucide-react';
...
export const ReportTemplateGallery: React.FC<ReportTemplateGalleryProps> = ({ onTemplateUsed }) => {
  ...
  const [sharingReport, setSharingReport] = useState<SavedReport | null>(null);
  ...
  const currentUserId: string | undefined = meResp?.data?.id;
  ...
  return (
    <>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(template => {
            const isOwner = !!currentUserId && template.ownerId === currentUserId;
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  {/* Share button — only visible for owned templates */}
                  {isOwner && (
                    <button
                      onClick={() => setSharingReport(template)}
                      aria-label={`Share "${template.name}"`}
                      title="Share template"
                      className={`p-1.5 rounded-lg transition-all duration-300 ${
                        (template.sharedWithRoles?.length ?? 0) > 0 || template.isPublic
                          ? 'text-nesma-secondary bg-nesma-primary/15'
                          : 'text-gray-500 hover:text-nesma-secondary hover:bg-white/10'
                      }`}
                    >
                      <Share2 size={14} />
                    </button>
      ...
      {/* Share modal */}
      {sharingReport && <ShareTemplateModal report={sharingReport} onClose={() => setSharingReport(null)} />}
    </>
  );
};
